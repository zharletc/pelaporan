...
class m_laporan extends CI_Model {
    ...
    public function getLatestIklan($status) {
        $value = $this->db->get_where('latest_iklan', array('STATUS_LAPORAN' => $status));
        return $value;
    }
    ...
    public function getWLihatiklan($id_iklan, $status) {
        $value = $this->db->get_where('latest_iklan', array('ID_IKLAN' => $id_iklan, 'STATUS_LAPORAN' => $status));
        return $value;
    }
    public function getPostByMonth($date){
        $value = $this->db->get_where('latest_iklan',array('MONTH(TANGGAL)'=>$date));
